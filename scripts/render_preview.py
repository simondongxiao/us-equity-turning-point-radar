...
def main() -> None:
    ...
    if args.input:
        ...
        for seed in seeds:
            ...
            records.append({**seed,'as_of':None,'reference_price':None,'stage':'待分析','metrics':{str(h):{'status':'uncalibrated','opportunity_value':None,'risk_value':None,'opportunity_score':None,'risk_score':None,'bottom_band':None,'top_band':None,'p_bottom':None,'p_top':None,'p_upfirst':None,'p_downfirst':None,'p_unhit':None,'expected_return':None,'es95':None,'positive_edge':None} for h in [5,10,21]}})
        data={'build_mode':'preview','generated_at':datetime.now(timezone.utc).isoformat(),'as_of':None,'run_id':'seed-ui-preview-not-a-forecast','model_version':None,'universe_version':'curated-seed-20260915-v1.1','taxonomy_version':'research-taxonomy-20260915-v1.1','research_taxonomy':json.loads((ROOT/'assets/research_taxonomy.json').read_text(encoding='utf-8')),'storage_rotation':None,'public_config':{'api_base_url':None},'records':records,'temporary':[]}
